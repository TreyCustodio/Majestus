...
class TextEngine(object):
    ...
    class _TE(object):
        ...
        def __init__(self):
            ...
            self.buffTime = 0.2
            self.fast_tick = 0 # Controls how often to play the text sound while speeding thru text
            self.max_fast_tick = 4 # Number of frames needed to pass before the text sound plays during speed up
        ...
        def parseChar(self, char = "", drawSurface = None):
            ...
            #   (2.) Invisible
            elif self.type == 4 or self.type == 2:
                ...
                #   (iii.) Waiting for input
                    ...
                    if next_char == "&":
                        ...
                        self.dialogue[self.lineNum - 1].append(("wait", "&"))
                        self.charIndex += 1
                        return
                ...
                if self.type == 2:
                    if self.lineNum == 2:
                        pos = ((8 * self.charIndex), 34)
                    elif "&&" in self.line:
                        pos = ((8 * self.charIndex), 22)
                    else:
                        pos = ((8 * self.charIndex), 7)

                    self.dialogue[self.lineNum - 1].append(Text.drawChar(drawSurface, pos, char= char, color=self.color, flag=2))
            ...
            #   (3.) Other Boxes
                ...
                ##  Draw the char onto the textBox  ##
                if self.lineNum == 2:
                    self.dialogue[self.lineNum - 1].append(Text.drawChar(drawSurface, char= char, color=self.color, flag=2))
                    #Text.drawChar(self.textBox, ((12) + (8 * self.charIndex), 34), char, flag=2)
                
                elif "&&" in self.line:
                    self.dialogue[self.lineNum - 1].append(Text.drawChar(drawSurface, char= char, color=self.color, flag=2))
                    #Text.drawChar(self.textBox, ((12) + (8 * self.charIndex), 22), char, flag=2)
                
                else:
                    self.dialogue[self.lineNum - 1].append(Text.drawChar(drawSurface, char= char, color=self.color, flag=2))
                    #Text.drawChar(self.textBox, ((12) + (8 * self.charIndex), 7), char, flag=2)
        ...
        def displayText(self, drawSurface, mainSurface): 
            ...
            # print("line", self.lineNum -1)
            # print("charIndex", self.charIndex)
            # print()
            ...
            if char == "clear":
                ...
                if not self.noSound:
                    if EventManager.getInstance().isPressed("interact"):
                        if self.fast_tick == self.max_fast_tick:
